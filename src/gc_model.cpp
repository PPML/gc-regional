...
void matReportOnNans(mat x, int y, int w, std::string name) {
  if (matHasNans(x)) {
    cout << "year: " << y << " week: " << w << " ";
    cout << name << " has nans\n";
  }
...
void reportOnNans(vec x, int y, int w, std::string name) {
  if (vectorHasNans(x)) {
    cout << "year: " << y << " week: " << w << "\n";
    cout << name << " has nans\n";
  }
...
Rcpp::List gcSim(
  ...
  arma::vec gamma(dim); // natural cure rate for symptomatic cases
  arma::vec delta(dim); // natural cure rate for asymptomatic cases
  arma::vec pS(dim); // probability symptomatic
    ...
    for(int w=0; w<52; w++){
      ...
      if(y==0 && w==0 && d==0){   ///populate the model at t=0
        ...
        for(int n=0; n<4; n++) { //males subpop 1
          int m = n;
          lambda[n] =  ((b[0] + (1.0-b[0])*c_rr_hetero*increase_risk_all_pops_int) * (cmlow1[m]     * (Y[16] + Z[16]) / (n_sa[16]) + //F, low AC, age=1, subpop1
        ...
        for(int n=4; n<8; n++) {   //males subpop 2
          int m = n-4;
          lambda[n] =   ((b[0] + (1.0-b[0])*c_rr_hetero*increase_risk_all_pops_int) * (cmlow2[m]     * (Y[16] + Z[16]) / (n_sa[16]) + //F, low AC, age=1, subpop1
        ...
        for(int n=8; n<12; n++) {   //males subpop 3
          int m = n-8;
          lambda[n] =   ((b[0] + (1.0-b[0])*c_rr_hetero*increase_risk_all_pops_int) * ( cmlow3[m]     * (Y[16] + Z[16]) / (n_sa[16]) + //F, low AC, age=1, subpop1
        ...
        for(int n=12; n<16; n++) {   //males subpop 4
          int m = n-12;
          lambda[n] =  (((b[0] + (1.0 - b[0])*c_rr_hetero*increase_risk_all_pops_int) *  (
        ...
        for(int n=16; n<20; n++) {   //females subpop 1
          int m = n;
          lambda[n] =  ((b[1] + (1.0-b[1])*c_rr_hetero*increase_risk_all_pops_int) * (cmlow1[m+16]     * (Y[0]  + Z[0])  /(n_sa[0])  + //M, low AC, age=1, subpop1
        ...
        for(int n=20; n<24; n++) {   //females subpop 2
          int m = n-4;
          lambda[n] =  ((b[1] + (1.0-b[1])*c_rr_hetero*increase_risk_all_pops_int) * (cmlow2[m+16]     * (Y[0]  + Z[0])  /(n_sa[0])  + //M, low AC, age=1, subpop1
        ...
        for(int n=24; n<28; n++) {   //females subpop 3
          int m = n-8;
          lambda[n] =  ((b[1] + (1.0-b[1])*c_rr_hetero*increase_risk_all_pops_int) *  ( cmlow3[m+16]     * (Y[0]  + Z[0])  /(n_sa[0])  + //M, low AC, age=1, subpop1
