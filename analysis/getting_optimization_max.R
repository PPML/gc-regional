...
output_directory <-
  "~/Documents/gcRegional/output/09-20-18/"
...
for (site in c("SF", "BA")) {
  ...
  f_list <-
    grep(
      ...
      list.files(paste0(output_directory, "optim/"), full.names = T),
      value = T)
  ...
  keepers <- which(dLogPosterior_vals == max(dLogPosterior_vals))
  trace <- do.call(rbind, lapply(1:20, function(x) trace[keepers, ]))
  ...
  plot_posteriors(output_directory = output_directory,
                  filename = paste0(site, "_max_optim_", ".pdf"))
